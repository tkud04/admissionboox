    ...
    <div class="utf_dashboard_list_box invoices with-icons margin-top-20">
      ...
      <ul>
        <?php
         ...
         {
          $iu = url('view-email')."?xf=".$se['id'];
        ?>
        <li><i class="utf_list_box_icon sl sl-icon-doc"></i> <strong>{{ucwords($se['title'])}}</strong>
        <ul>
        <li>
          <p>
            <span>Date sent:-</span> {{$se['date']}}<br>
            <span>No. of applicants:-</span> {{$se['num_applicants']}}
          </p>
        </li>
        </ul>
        <div class="buttons-to-right"> <a href="{{$iu}}" target="_blank" class="button gray"><i
          class="sl sl-icon-eyeglass"></i> View</a> </div>
      </li>
       <?php
         }
        }
        ...
        {
        ?>
         <li><i class="utf_list_box_icon sl sl-icon-note"></i> No emails sent... yet. <a href="{{url('send-email')}}">Send a message</a></strong>
      </li>
        <?php
        }
       ?>
